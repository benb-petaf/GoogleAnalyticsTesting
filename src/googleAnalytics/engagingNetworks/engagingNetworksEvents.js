...
var takeActionPhrases = new Array('Take Action', 'Kom in actie', 'Agissez', 'アクションを起こす', '現在就行動', 'Toma acción', 'Actúa');
var tafPhrases = new Array('Tell-a-Friend', 'Verteel en vriend', 'Vertel een vriend', 'Passez le mot', '友達に教える', '与朋友分享', 'Díselo a un amigo');
...
else if( checkForTAF() == true )
{
	...
	if( isPage1 == true )
	{
		sendEvent(petaGA_FormCategory_ForwardToAFriend, petaGA_FormAction_Initiated, currentPageTitle);
	}
	// "Tell A Friend" Submission Event
	else if( isPage2 == true )
	{
		sendEvent(petaGA_FormCategory_ForwardToAFriend, petaGA_FormAction_Submitted, currentPageTitle);
	}
}
...
else if( checkForDonate() == true )
{
	...
	if( isPage1 == true )
	{
		sendEvent(petaGA_FormCategory_Donate, petaGA_FormAction_Initiated, currentPageTitle);
	}
